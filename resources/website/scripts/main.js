var MAIN = (function(){
	...
	function attemptLogin(){
		if(window.SONGLIST_CONFIG.COGNITO_LOGIN_BASE_URL_STR !== null){
			window.location = window.SONGLIST_CONFIG.COGNITO_LOGIN_BASE_URL_STR;
		}else{
	...
	function changeImage(){
		var album_name_str = "apple_pie";

		if(document.cookie !== "" && document.cookie.split('=')[1] !== ""){
			album_name_str_name_str = document.cookie.split("=")[1];
		}
		$("[data-role2='special_highlight']")
				.css({
					"background-image": "url(/images/album/" + album_name_str + ".jpg)"
				});
	}
	...
	function establishAndHandleCognitoStatus(){
		if(window.SONGLIST_CONFIG.COGNITO_LOGIN_BASE_URL_STR !== null){
			...
			$(document).on("click", "[data-role='login_button']", function(){
				location.href = window.SONGLIST_CONFIG.COGNITO_LOGIN_BASE_URL_STR;
			});
	...
	function attemptCreateReport(ce){
		...
		$.ajax({
			url: window.SONGLIST_CONFIG.API_GW_BASE_URL_STR + "/" + "create_report",
			method: "POST",
			data: {},
			headers: {
