var COFEEE = (function(){
	...
	(function init(){
		console.log("Ok lets start members");
		loadItems();
	})();
	...
	function handleNoSupplierSetUpYet(msg_str){
		...
		html_str += 	'</output>';
		// html_str +=	'</section>';
		$("[data-role='browse_songlist_content']")
				.append(html_str);
	}

	function loadItems(){
		if(window.SONGLIST_CONFIG.API_GW_BASE_URL_STR === null){
			handleNoSupplierSetUpYet("Members information coming shortly");
		}else{
			$.get(window.SONGLIST_CONFIG.API_GW_BASE_URL_STR + "/bean_products", printBeans)
				.fail(function(){
					handleNoSupplierSetUpYet("Members supply information coming <u>very soon</u>!");
				});
		}
	}
	...
	function printItems(all_products_obj_arr){
		...
		for(var i_int = 0, o = {}; i_int < all_products_obj_arr.length; i_int += 1){
			...
			html_str += 	'</section>';
			html_str += 	'<figure>';
			html_str += 		'<img src="images/album/' + formatWithUnderscores(o.product_name_str) + '.jpg" alt="Image for our ' + o.product_name_str + '" />';
			html_str += 		'<figcaption>';
		...
		html_str += '</section>';
		$("[data-role='browse_songlist_content']").append(html_str);
	}
